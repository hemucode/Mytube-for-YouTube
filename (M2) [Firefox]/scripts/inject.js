...
const WEBSTORE = `https://addons.mozilla.org/en-US/firefox/addon/mytube-youtube-theme/`;
...
class Dialog {
  ...
  start() {
    ...
    try {
      setTimeout(() => {
        if (this.updateVersionEdge > browser.runtime.getManifest().version) {
          ...
          if (this.isRateUsPopupEnabled && this.nextRatingRequest) {
            const webstoreLink = this.createDialogPopup(browser.i18n.getMessage("rate") , WEBSTORE_REVIEW);
  ...
  createDialogPopup(text, link) {
    ...
    header.appendChild(
      document.createTextNode(browser.i18n.getMessage("app_name"))
    );
...
async function info(adBlockSelector){
   var promise = new Promise((resolve, reject)=> {
      browser.storage.local.get({
        ...
        "updateVersionEdge": browser.runtime.getManifest().version,
        "isMessage": "",
        "isMessageURL": "",

...
class Dialog {
  ...
  constructor(updateVersionChrome,
    isRateUsPopupEnabled, 
    nextRatingRequest, 
    isMessage, 
    isMessageURL) {
      this.nextRatingRequest = nextRatingRequest;
      this.updateVersionChrome = updateVersionChrome;
  ...
  start() {
    ...
    try {
      setTimeout(() => {
        if (this.updateVersionChrome > chrome.runtime.getManifest().version) {
          this.createDialogPopup(`New Version ${this.updateVersionChrome} Available Please Update..😉`, WEBSTORE);
        }else{
...
async function info(adBlockSelector){
   var promise = new Promise((resolve, reject)=> {
      chrome.storage.sync.get({
        ...
        "updateVersionChrome": chrome.runtime.getManifest().version,
        "isMessage": "",
        "isMessageURL": "",
  ...
  try {
    ...
    const filters = [
      ...
      new Dialog(options.updateVersionChrome, options.isRateUsPopupEnabled,  options.nextRatingRequest, options.isMessage, options.isMessageURL)
    ];
